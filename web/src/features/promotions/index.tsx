...
function PromotionDrawer({
  ...
  useEffect(() => {
    ...
    if (currentRow) {
      ...
      setLandingContentFormat(currentRow.landing_content_format ?? 'plain_text')
      ...
      setExpiresAt(
        currentRow.expires_at
          ? new Date(currentRow.expires_at * 1000)
          : undefined
      )
            ...
            <Select
              ...
              onValueChange={(value) =>
                setLandingContentFormat(
                  value as 'plain_text' | 'markdown' | 'html'
                )
              }
            >
              <SelectTrigger className='w-full'>
  ...
  const columns = useMemo<ColumnDef<PromotionRegistration>[]>(
    () => [
      ...
      { accessorKey: 'channel_tag', header: t('Channel Tag') },
      {
        ...
        cell: ({ row }) =>
          t(getPromotionActivityLabel(row.original.activity_type)),
      },
      {
        accessorKey: 'reward_status',
        header: t('Reward Status'),
        cell: ({ row }) =>
          t(getPromotionRewardStatusLabel(row.original.reward_status)),
      },
      {
