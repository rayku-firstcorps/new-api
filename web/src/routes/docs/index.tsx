...
function createClientGuides(gatewayBaseUrl: string): ClientGuide[] {
...
name = "new-api"
base_url = "${gatewayV1Url}"
requires_openai_auth = true
wire_api = "responses"`,
      run: 'codex',
      summary:
        'Use the OpenAI Responses route and set the base URL to the gateway /v1 endpoint.',
    },
    {
      id: 'cursor',
      protocol: 'openai',
...
Model: gpt-5.4`,
  ...
  "env": {
    ...
    "CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS": "1"
      ...
      run: 'claude',
      summary:
        'Use the Anthropic Messages route and keep the base URL at the gateway root.',
    },
    {
      id: 'opencode',
      protocol: 'claude',
  ...
  "provider": {
    "new-api": {
      ...
      run: 'opencode',
      summary:
        'Use the Anthropic SDK provider and point it to the gateway route.',
    },
    {
      id: 'cherry',
      protocol: 'gemini',
...
Model: gemini-3.1-pro`,
      ...
      summary:
        'Use the Gemini route for desktop clients that support custom Gemini endpoints.',
    },
  ]
}

function createRouteOptions(gatewayBaseUrl: string) {
  const gatewayV1Url = `${gatewayBaseUrl}/v1`
  const host = getDisplayHost(gatewayBaseUrl)
...
const faqItems = [
  {
    ...
    answer:
      'OpenAI-compatible clients usually use /v1. Claude and Gemini clients usually use the gateway root unless the client asks for a versioned endpoint.',
  },
  {
    question: 'Where do I get the API key?',
    answer:
      'Create a dedicated key in the console, then limit its group, quota, model permissions, and rate limits for CLI tools.',
  },
  {
    question: 'How do I verify the setup?',
    answer:
      'Run one short request, then check usage logs for model, channel, status, and token usage.',
  },
]
...
function RouteSelector({
  ...
  routeOptions: ReturnType<typeof createRouteOptions>
}) {
  const { t } = useTranslation()
  ...
  return (
    <div className='max-w-full min-w-0 space-y-7 overflow-hidden'>
      <div className='border-border/70 bg-background w-full max-w-full min-w-0 rounded-xl border p-5'>
        <div className='flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between'>
          <div className='min-w-0'>
            ...
            <h1 className='mt-2 text-2xl font-semibold tracking-tight break-words md:text-3xl'>
              {guide.name} · {guide.model}
            </h1>
            <p className='text-muted-foreground mt-3 max-w-2xl text-sm leading-7 break-words'>
              {t(guide.summary)}
            </p>
          </div>
          <div className='border-border/70 bg-muted/30 flex max-w-full min-w-0 items-center gap-2 self-start rounded-full border px-3 py-2 text-xs font-medium sm:max-w-[45%]'>
            <Server className='size-3.5 text-emerald-600' />
            <span className='min-w-0 truncate'>{gatewayBaseUrl}</span>
          </div>
        </div>
      </div>
      ...
      <div className='max-w-full min-w-0 space-y-7 overflow-hidden'>
        ...
        <StepRow
          index={2}
          title='Run the version command to confirm the client is available.'
        >
        ...
        <StepRow
          index={4}
          title='Write the following configuration and save the file.'
        >
        ...
        <StepRow
          index={6}
          title='Check usage logs and confirm the request route is recorded.'
        >
          <div className='border-border/70 flex max-w-full min-w-0 items-start gap-3 rounded-xl border bg-emerald-50/60 p-4 text-sm leading-6 dark:bg-emerald-950/20'>
...
function DocsPage() {
  ...
  const clientGuides = useMemo(
    () => createClientGuides(gatewayBaseUrl),
    [gatewayBaseUrl]
  )
  const routeOptions = useMemo(
    () => createRouteOptions(gatewayBaseUrl),
    [gatewayBaseUrl]
  )

  const visibleClients = useMemo(
    () => clientGuides.filter((client) => client.protocol === activeProtocol),
    [activeProtocol, clientGuides]
  )
  const activeGuide =
  ...
  const selectProtocol = (protocol: ProtocolId) => {
    ...
    const firstClient = clientGuides.find(
  ...
  return (
    <PublicLayout showMainContainer={false}>
      <main className='bg-background min-h-svh w-full max-w-[100vw] overflow-x-hidden pt-16'>
        <div className='border-border/70 bg-background/95 border-b backdrop-blur-xl'>
        ...
        <div className='mx-auto grid w-full max-w-7xl min-w-0 gap-8 px-4 pt-10 pb-8 md:px-6 lg:grid-cols-[minmax(0,1fr)_260px]'>
          <article className='w-full max-w-full min-w-0 overflow-hidden'>
            {activeProtocol === 'faq' || !activeGuide ? (
              <FaqBody />
            ) : (
              <GuideBody gatewayBaseUrl={gatewayBaseUrl} guide={activeGuide} />
            )}
          </article>

          <div className='min-w-0 lg:sticky lg:top-40 lg:self-start'>
            <RouteSelector routeOptions={routeOptions} />
