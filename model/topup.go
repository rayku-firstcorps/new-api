...
func RechargePayssion(tradeNo string, callerIp string) (err error) {
	if tradeNo == "" {
		return errors.New("未提供支付单号")
	}
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		if topUp.PaymentProvider != PaymentProviderPayssion {
			return ErrPaymentMethodMismatch
		}
	...
	if err != nil {
		common.SysError("payssion topup failed: " + err.Error())
		return errors.New("充值失败，请稍后重试")
	}
